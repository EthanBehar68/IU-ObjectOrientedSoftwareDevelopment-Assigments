...
import gamemaker.model.actions.MoveAction;
...
import gamemaker.model.event.CollisionEventHandler;
import gamemaker.model.event.InputEventHandler;
...
public class Model implements Savable, Dumpable {
	...
	private void buildSpriteEventLists(Sprite newSprite) {
		...
		timeEventsToSpriteMap.get(newSprite.getId())
				.add(new TimeEvent(newSprite.getId(), -1, new MoveAction(newSprite, null)));
	...
	public void updateSelectedSpritesEvents(LinkedList<TimeEvent> timeEvents, LinkedList<KeyCodeEvent> keyEvents,
		...
		timeEventsToSpriteMap.get(currentSelectedSprite.getId())
				.add(new TimeEvent(currentSelectedSprite.getId(), -1, new MoveAction(currentSelectedSprite, null)));
...
//		target.setLayoutY(layoutY);
	...
	public void playGame() {
		...
		collisionEventHandler = new CollisionEventHandler(collisionEventsToSpriteMap.keySet().iterator(),
				collisionEventsToSpriteMap);
		gameEngine.registerObserver(collisionEventHandler);
