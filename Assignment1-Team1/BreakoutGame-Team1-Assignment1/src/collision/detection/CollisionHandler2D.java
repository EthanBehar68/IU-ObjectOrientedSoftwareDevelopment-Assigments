...
//	}
    ...
    private void checkScreenBoundsCollision(GameObject object) {
		...
		else if (velocity.getX() > 0 && (nextPosition.getX() + dimensions.getX()) >= gameScene.getWidth()) { 
			positionX = gameScene.getWidth() - dimensions.getX();
		} 
		...
		else if (velocity.getY() > 0 && (nextPosition.getY() + dimensions.getY()) >= gameScene.getHeight()) { 
			positionY = gameScene.getHeight() - dimensions.getY();
		} 
