...
public class CollisionDetectionUtility {
	/**
	 * Determines the collision type by checking where the collision happened. No
	 * collision occurs if either object is marked as destroyed.
	 * 
	 * @param impactee the object that collider might be hitting/touching since
	 *                 collider object is moving
	 * @return any one of the possible CollisionTypes. Ex: Return LeftImpact if the
	 *         collider hits the impactee's left side.
	 */
	public static CollisionType checkForCollision(Sprite collider, Sprite impactee) {
		...
		// If the collider's y-velocity is so fast that it will speed right through the
		// impactee...
		if (Math.abs(colliderVelY) > impacteeHeight) {
			if (colliderX >= impacteeX && colliderX <= impacteeX + impacteeWidth) {
				if (colliderY < impacteeY && colliderY + colliderVelY >= impacteeY)
					return CollisionType.TOP_IMPACT;
				else if (colliderY > impacteeY && colliderY + colliderVelY <= impacteeY)
					return CollisionType.BOTTOM_IMPACT;
			}
		}
		// If the collider's x-velocity is so fast that it will speed right through the
		// impactee...
			...
			if (colliderY >= impacteeY && colliderY <= impacteeY + impacteeHeight) {
				if (colliderX < impacteeX && colliderX + colliderVelX >= impacteeX)
					return CollisionType.RIGHT_IMPACT;
				else if (colliderX > impacteeX && colliderX + colliderVelX <= impacteeX)
					return CollisionType.LEFT_IMPACT;
			}
		}

		// Check if the collider is moving more vertically than horizontally. If so, it
		// may be a vertical impact.
	...
	private static CollisionType checkForVerticalCollision(Sprite collider, Sprite impactee) {
		...
		double impacteeX = impactee.getX();
		double impacteeY = impactee.getY();
		double colliderWidth = collider.getView().getSpriteWidth(); // TODO this was unused previously; check if
		...
		if (colliderX >= impacteeX - colliderWidth && colliderX <= impacteeX + impacteeWidth) {
			...
			if (colliderY + colliderHeight >= impacteeY
					&& colliderY + colliderHeight <= impacteeY + (impacteeHeight / 2))
				return CollisionType.TOP_IMPACT;
	...
	private static CollisionType checkForHorizontalCollision(Sprite collider, Sprite impactee) {
		...
		if (colliderY >= impacteeY - colliderHeight && colliderY <= impacteeY + impacteeHeight) {
			...
			else if (colliderX >= impacteeX + impacteeWidth && colliderX <= impacteeX + impacteeWidth)
				return CollisionType.RIGHT_IMPACT;
		}
	...
	/**
	 * Checks each Sprite in the list to see if it collides with (is touching) any
	 * other Sprite in the list. If it is, fires the object's collision behavior
	 * methods.
	 * 
	 * @param allSprites a list of all Sprites in the game that should have their
	 *                   collisions checked
	 */
	public static void checkForCollisions(List<Sprite> allSprites) {
		/*
		 * Check for collisions... Define two iterators to iterate through allSprites.
		...
		while (colliderIterator.hasNext()) {
			...
			while (impacteeIterator.hasNext()) {
				...
				if (i != j) {
					...
					if (collisionType != CollisionType.NO_COLLISION) {
